...
public class ASTDynamicJsonPrint implements Printer {
    private PrintWriter writer = null;
    private ASTDynamic ast = null;
    private boolean firstline = true;

    public ASTDynamicJsonPrint(ASTDynamic ast, File f) throws FileNotFoundException {
    ...
    public void visit() {
        ...
        for(Integer key : ast.getNodeIds()) {
            ...
            if(ast.getEvalString(key) != null)
                print("eval_code", key, ast.getEvalString(key));
            if(ast.getPatternsMatched(key) != null)
                for(Integer pattern : ast.getPatternsMatched(key))
                    print("eval_pattern", key, pattern);
            ...
            for(Integer childId : ast.getDynamicResolution(key))
                print("dynres_succ", key, childId);
            for(Integer childId : ast.getDataflowResolution(key))
                print("datares_succ", key, childId);
            for(Integer childId : ast.getParseEdge(key))
                print("parse_succ", key, childId);
